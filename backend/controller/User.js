...
userRegister:async (req, res)=>{
  ...
  await newUser.save();
  res.status(201).json({message:"User registered successfully" , recoveryKey:recoveryKey})
 ...
 completeProfile :async (req, res)=>{
    ...
    try{
      ...
      if(!checkUser){
       ...
       if(checkEmail){
        return res.status(400).json({message:"Nominee email already exists"})
       }
       checkUser.nomineeName = nomineeName;
       checkUser.nomineeEmail = nomineeEmail;
       checkUser.checkInFrequency = checkInFrequency;
